"""Hypothesis Manager - Complete Hypothesis Lifecycle Management

Consolidated hypothesis management logic for the milestone-based investigation framework.

Ported from: FaultMaven-Mono/faultmaven/core/investigation/hypothesis_manager.py
...
class HypothesisManagerError(Exception):
    """Exception raised by HypothesisManager"""
    pass
...
class HypothesisManager:
    ...
    Source: FaultMaven-Mono hypothesis_manager.py (752 LOC)
    """
    ...
    def create_hypothesis(
        ...
        Returns:
            New HypothesisModel object

        Source: FaultMaven-Mono lines 68-119
        """
        ...
        self.logger.info(
            ...
            f"{statement[:50]}... (category={category}, likelihood={initial_likelihood:.2f}, "
    ...
    def link_evidence(
        ...
        Source: FaultMaven-Mono lines 121-158
        """
        if supports:
    ...
    def update_confidence_from_evidence(
        ...
        Progress detection:
        - Confidence change >= 0.05 (5% threshold) counts as progress
        - Otherwise increment iterations_without_progress
        ...
        # Calculate new confidence using evidence-ratio formula
        ...
        # Check if this represents progress (5% threshold)
        ...
        self.logger.info(
            ...
            extra={
                ...
                "progress": abs(new_confidence - old_confidence) >= 0.05,
            },
        )
    ...
    def update_hypothesis_confidence(
        ...
        Source: FaultMaven-Mono lines 218-259
        """
    ...
    def _check_status_transition(
        ...
        - RETIRED: confidence < 0.30 (low confidence after testing)
        ...
        elif hypothesis.likelihood < 0.30:
    ...
    def apply_confidence_decay(
        ...
        hypothesis.likelihood = old_likelihood * decay_factor

        # Clamp to valid range
        hypothesis.likelihood = max(0.0, min(1.0, hypothesis.likelihood))
        ...
        # Check if decay caused auto-transition
        self._check_status_transition(hypothesis, current_turn)
    ...
    def detect_anchoring(
        ...
        # Condition 2: No progress for 3+ iterations on multiple hypotheses
        stalled_hypotheses = [
            h.hypothesis_id
            for h in active_hypotheses
    ...
    def force_alternative_generation(
        self,
        existing_hypotheses: List[HypothesisModel],
        current_turn: int,
    ) -> Dict[str, Any]:
        ...
        for h in existing_hypotheses:
            if (
                ...
                h.validated_at_turn = current_turn
                retired_count += 1
    ...
    def get_testable_hypotheses(
        ...
        Source: FaultMaven-Mono lines 573-602
        """
        testable = [
            h
            for h in hypotheses
    ...
    def get_validated_hypothesis(
        ...
        Source: FaultMaven-Mono lines 604-626
        """
        validated = [
            h
            for h in hypotheses
    ...
    def get_hypothesis_summary(
        ...
        Source: FaultMaven-Mono lines 628-675
        """
        summary = {
        ...
        summary["opportunistic"] = sum(
            ...
            if h.generation_mode == HypothesisGenerationMode.OPPORTUNISTIC.value
        )
        summary["systematic"] = sum(
            1 for h in hypotheses
            if h.generation_mode == HypothesisGenerationMode.SYSTEMATIC.value
        )
        summary["forced_alternative"] = sum(
            1 for h in hypotheses
            if h.generation_mode == HypothesisGenerationMode.FORCED_ALTERNATIVE.value
        )
    ...
    def get_best_hypothesis(
        ...
        Source: FaultMaven-Mono lines 677-697
        """
        active_hypotheses = [
    ...
    def get_hypotheses_by_category(
        ...
        Source: FaultMaven-Mono lines 699-713
        """
    ...
    def has_validated_hypothesis(
        ...
        Source: FaultMaven-Mono lines 715-730
        """
        return any(
...
def rank_hypotheses_by_likelihood(hypotheses: List[HypothesisModel]) -> List[HypothesisModel]:
    ...
    Source: FaultMaven-Mono lines 742-751
    """
